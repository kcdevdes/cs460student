...
THREE.GLTF2Loader = (function () {
        ...
        function (data) {
          try {
            ...
            onError(
              e.constructor === Error
                ? e
                : new Error('THREE.GLTF2Loader: Unable to parse model.')
            );
          }
        },
        onProgress,
    ...
    parse: function (data, path, onLoad, onError) {
      ...
      if (json.extensionsUsed) {
        ...
        if (json.extensionsUsed.indexOf(EXTENSIONS.KHR_MATERIALS_COMMON) >= 0) {
          extensions[EXTENSIONS.KHR_MATERIALS_COMMON] =
            new GLTFMaterialsCommonExtension(json);
        }

        if (
          json.extensionsUsed.indexOf(
        ...
        if (json.extensionsUsed.indexOf(EXTENSIONS.KHR_TECHNIQUE_WEBGL) >= 0) {
          extensions[EXTENSIONS.KHR_TECHNIQUE_WEBGL] =
            new GLTFTechniqueWebglExtension(json);
        }
      }
  ...
  GLTFShader.prototype.update = function (scene, camera) {
    ...
    for (var name in boundUniforms) {
      ...
      switch (boundUniform.semantic) {
        ...
        default:
          console.warn(
            'THREE.GLTF2Loader: Unhandled shader semantic: ' +
              boundUniform.semantic
          );
          break;
      }
    }
  ...
  GLTFMaterialsCommonExtension.prototype.extendParams = function (
    materialParams,
    material,
    dependencies
  ) {
  ...
  function GLTFTechniqueWebglExtension(json) {
    ...
    var extension =
      (json.extensions && json.extensions[EXTENSIONS.KHR_TECHNIQUE_WEBGL]) ||
      {};
  ...
  GLTFTechniqueWebglExtension.prototype.extendParams = function (
    materialParams,
    material,
    dependencies
  ) {
    ...
    if (!materialParams.fragmentShader) {
      throw new Error(
        'THREE.GLTF2Loader: Missing fragment shader definition: ' +
    ...
    if (!vertexShader) {
      throw new Error(
        'THREE.GLTF2Loader: Missing vertex shader definition: ' +
      ...
      if (WEBGL_TYPE[ptype]) {
        ...
        switch (ptype) {
          ...
          case WEBGL_CONSTANTS.FLOAT_MAT2:
            // what to do?
            console.warn(
              'THREE.GLTF2Loader: FLOAT_MAT2 is not a supported uniform type.'
            );
            break;
        ...
        throw new Error(
          'THREE.GLTF2Loader: Unknown shader uniform param type: ' + ptype
        );
      }
    }
    ...
    for (var i = 0, il = enables.length; i < il; i++) {
      ...
      switch (STATES_ENABLES[enable]) {
        ...
        default:
          throw new Error(
            'THREE.GLTF2Loader: Unknown technique.states.enable: ' + enable
          );
      }
    }
    ...
    return {
      ...
      extendParams: function (params, material, dependencies) {
        ...
        if (pbrSpecularGlossiness.diffuseTexture !== undefined) {
          params.map =
            dependencies.textures[pbrSpecularGlossiness.diffuseTexture.index];
        }
  ...
  GLTFParser.prototype.parse = function (onLoad, onError) {
    ...
    this._withDependencies(['scenes', 'cameras', 'animations'])
      .then(function (dependencies) {
        ...
        var scene =
          json.scene !== undefined
            ? dependencies.scenes[json.scene]
            : scenes[0];
  ...
  GLTFParser.prototype.loadShaders = function () {
    ...
    return this._withDependencies(['bufferViews']).then(function (
      ...
      var shaders =
        extensions[EXTENSIONS.KHR_TECHNIQUE_WEBGL] !== undefined
          ? extensions[EXTENSIONS.KHR_TECHNIQUE_WEBGL].shaders
          : json.shaders;
    ...
    return this._withDependencies(['shaders', 'textures']).then(function (
      ...
      return _each(json.materials, function (material) {
        ...
        if (materialExtensions[EXTENSIONS.KHR_MATERIALS_COMMON]) {
          materialType =
            extensions[EXTENSIONS.KHR_MATERIALS_COMMON].getMaterialType(
              material
            );
          extensions[EXTENSIONS.KHR_MATERIALS_COMMON].extendParams(
            materialParams,
            material,
            dependencies
          );
          ...
          materialType =
            extensions[EXTENSIONS.KHR_TECHNIQUE_WEBGL].getMaterialType(
              material
            );
          extensions[EXTENSIONS.KHR_TECHNIQUE_WEBGL].extendParams(
            materialParams,
            material,
            dependencies
          );
          ...
          if (metallicRoughness.baseColorTexture !== undefined) {
            materialParams.map =
              dependencies.textures[metallicRoughness.baseColorTexture.index];
          }
        ...
        if (material.normalTexture !== undefined) {
          materialParams.normalMap =
            dependencies.textures[material.normalTexture.index];
        }

        if (material.occlusionTexture !== undefined) {
          materialParams.aoMap =
            dependencies.textures[material.occlusionTexture.index];
        }
        ...
        if (material.emissiveTexture !== undefined) {
          if (materialType === THREE.MeshBasicMaterial) {
            materialParams.map =
              dependencies.textures[material.emissiveTexture.index];
          } else {
      ...
      return _each(json.meshes, function (mesh) {
        ...
        for (var name in primitives) {
          ...
          if (
            ...
            if (primitive.targets !== undefined) {
              ...
              for (var i = 0, il = targets.length; i < il; i++) {
                ...
                if (target.POSITION !== undefined) {
                  ...
                  positionAttribute =
                    dependencies.accessors[target.POSITION].clone();
                ...
                if (target.NORMAL !== undefined) {
                  ...
                  normalAttribute =
                    dependencies.accessors[target.NORMAL].clone();
            ...
            throw new Error(
              'THREE.GLTF2Loader: Only triangular and line primitives are supported.'
            );
          }
  ...
  GLTFParser.prototype.loadNodes = function () {
    ...
    return _each(json.nodes, function (node) {
      ...
      var _node =
        node.isBone === true ? new THREE.Bone() : new THREE.Object3D();
          ...
          return _each(__nodes, function (_node, nodeId) {
            ...
            if (meshes !== undefined) {
              for (var meshId in meshes) {
                ...
                if (group === undefined) {
                  console.warn(
                    'THREE.GLTF2Loader: Could not find node "' + mesh + '".'
                  );
                  continue;
                }
